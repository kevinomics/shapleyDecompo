...
The function `shapleyDecompo()` is wrap function of several functions used to compute the decomposition.

The function `getFactorList()` is used to extract the list of factors in the model.
```{r getFactorList}
...
The function `getCoalitions()` is used to obtain all possible coalitions from a list of factors.
```{r getCoalitions}
coa <- getCoalitions(factors_list = factors)
coa
```

The function `getShapleyDistrib()` is used to obtain the model matrix needed to perform the decomposition.
```{r getShapleyDistrib}
...
The function `getInequality()` is used to compute the inequality for a given coalition.  Several functions in the package can be used to compute corresponding measures (see `?Atkinson()`, `?Gini_G()`, `?Gini_w()`, `?Var()`, `?Entropy()`, `?Kolm()`).
```{r getInequality}
...
ineq
```
The function `getMarginalContrib()` can be used to compute the marginal contributions of each attribute in all coalitions.
...
margContrib
```
The function `getShapleyVal()` can be used to compute the Shapley values from marginal contributions of attributes.
```{r }
getShapleyVal(marginalContrib = margContrib)
```
The function `getInteractionTerms()` can be used to extract the interaction terms from the marginal contributions of attributes in each coalition.
